...
public class EmailProcessingService {
    ...
    public void processEmail(EmailRequest emailRequest) {
        SendEmailResponse response = vendorClient.sendEmailWithResilience(emailRequest);

        if (response.getStatus() >= 200 && response.getStatus() < 300) {
            txManager.updateNotificationStateAndLog(emailRequest.getNotificationId());
        } else {
            log.error("Notification ID {} explicitly rejected by provider gateway. Status: {}, Message: {}",
                    emailRequest.getNotificationId(), response.getStatus(), response.getMessage());
